...
test.describe("Consultas", () => {
  ...
  test("deve abrir modal de nova consulta", async ({ page }) => {
    ...
    await expect(page.locator('input[name="consulta-data"]')).toBeVisible({
      timeout: 5000,
    });
  ...
  test("deve criar consulta simples", async ({ page }) => {
    ...
    // Modal closes
    await expect(page.locator('input[name="consulta-data"]')).not.toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("deve criar consulta finalizada", async ({ page }) => {
    ...
    await expect(
      page.locator('textarea[name="consulta-diagnostico"]'),
    ).toBeVisible();
    await page.fill('textarea[name="consulta-diagnostico"]', "Enxaqueca");
    await page.fill('textarea[name="consulta-orientações"]', "Repouso");
    await page.click('button:has-text("Salvar Consulta")');
    await expect(page.locator('input[name="consulta-data"]')).not.toBeVisible({
      timeout: 10000,
    });
  ...
  test("deve mostrar checkboxes de medicamento e exame ao finalizar", async ({
    page,
  }) => {
    await page.locator('[data-testid="btn-nova-consulta"]').click();
    await page.locator('input[type="checkbox"]').first().check();
  ...
  test("deve clicar em consulta e ver detalhes", async ({ page }) => {
    ...
    if (await card.isVisible({ timeout: 5000 }).catch(() => false)) {
      await card.click();
      await expect(
        page.locator("text=Sintomas").or(page.locator("text=Status")),
